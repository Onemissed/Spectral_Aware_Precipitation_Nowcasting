...
from util.preprocess import padding_CIKM_data, reshape_patch, nor, de_nor, reshape_patch_back, unpadding_CIKM_data
...
def calculate_metrics(output, target, thresh, args):
    ...
    # return mse, mae, pod, far, csi, hss, ssim
    # return mse, mae, hits, misses, false_alarms, correct_negatives
...
class Model_eval_RNN(object):
    ...
    def eval(self, dataloader, model, epoch):
        ...
        output_length = self.args.total_length - self.args.input_length

        with torch.no_grad():
            with tqdm(total=len(dataloader)) as pbar:
                for i, X in enumerate(dataloader):
                    # 注意：这里要将Uint8的X变量转换为float类型
                    ...
                    next_frames *= 255
                    next_frames.clamp_(min=0, max=255)

                    img_out = reshape_patch_back(next_frames.cpu().numpy(), self.args.patch_size)
                    img_out = img_out[:, -output_length:]
            ...
            info = 'Test EPOCH INFO: epoch:{} \nMSE:{:.4f}  MAE:{:.4f}  SSIM:{:.4f}  PSNR:{:.4f}  MSE_MAE:{:.4f}\nCSI_16:{:.4f}  CSI_74:{:.4f}  CSI_133:{:.4f}  CSI_160:{:.4f}  CSI_181:{:.4f}  CSI_219:{:.4f}\nHSS_16:{:.4f}  HSS_74:{:.4f}  HSS_133:{:.4f}  HSS_160:{:.4f}  HSS_181:{:.4f}  HSS_219:{:.4f}\n'. \
                format(epoch + 1, mse_score, mae_score, ssim_score, psnr_score, mse_score + mae_score,
                       scores[16]['csi'], scores[74]['csi'], scores[133]['csi'], scores[160]['csi'], scores[181]['csi'],
                       scores[219]['csi'],
                       scores[16]['hss'], scores[74]['hss'], scores[133]['hss'], scores[160]['hss'], scores[181]['hss'],
                       scores[219]['hss'])
            print(info)
            ...
            print(
                "minMSE: {:.4f}  epoch:{}\nminMAE: {:.4f}  epoch:{}\nminMSE_MAE: {:.4f}  epoch:{}\nmaxSSIM: {:.4f}  epoch:{}\nmaxPSNR: {:.4f}  epoch:{}\nmaxCSI_16: {:.4f}  epoch:{}\nmaxCSI_74: {:.4f}  epoch:{}\nmaxCSI_133: {:.4f}  epoch:{}\nmaxCSI_160: {:.4f}  epoch:{}\nmaxCSI_181: {:.4f}  epoch:{}\nmaxCSI_219: {:.4f}  epoch:{}\nmaxHSS_16: {:.4f}  epoch:{}\nmaxHSS_74: {:.4f}  epoch:{}\nmaxHSS_133: {:.4f}  epoch:{}\nmaxHSS_160: {:.4f}  epoch:{}\nmaxHSS_181: {:.4f}  epoch:{}\nmaxHSS_219: {:.4f}  epoch:{}\nmaxAvgCSI: {:.4f}  epoch:{}\nmaxAvgHSS: {:.4f}  epoch:{}\n".format(
                    ...
                    self.max_metrics[16]['maxCSI'], self.max_metrics[16]['maxCSI_epoch'],
                    self.max_metrics[74]['maxCSI'], self.max_metrics[74]['maxCSI_epoch'],
                    self.max_metrics[133]['maxCSI'], self.max_metrics[133]['maxCSI_epoch'],
                    self.max_metrics[160]['maxCSI'], self.max_metrics[160]['maxCSI_epoch'],
                    self.max_metrics[181]['maxCSI'], self.max_metrics[181]['maxCSI_epoch'],
                    self.max_metrics[219]['maxCSI'], self.max_metrics[219]['maxCSI_epoch'],
                    self.max_metrics[16]['maxHSS'], self.max_metrics[16]['maxHSS_epoch'],
                    self.max_metrics[74]['maxHSS'], self.max_metrics[74]['maxHSS_epoch'],
                    self.max_metrics[133]['maxHSS'], self.max_metrics[133]['maxHSS_epoch'],
                    self.max_metrics[160]['maxHSS'], self.max_metrics[160]['maxHSS_epoch'],
                    self.max_metrics[181]['maxHSS'], self.max_metrics[181]['maxHSS_epoch'],
                    self.max_metrics[219]['maxHSS'], self.max_metrics[219]['maxHSS_epoch'],
