        ...
        with torch.no_grad():
            with tqdm(total=len(dataloader)) as pbar:
                for i, X in enumerate(dataloader):
                    ...
                    next_frames = reshape_patch_back(next_frames, self.args.patch_size)
                    next_frames = next_frames[:, :, 1:-2, 1:-2, :]
                    img_out = next_frames[:, -output_length:]

                    img_out *= 255
                    img_out.clamp_(min=0, max=255)
            ...
            info = 'Test EPOCH INFO: epoch:{} \nMSE:{:.4f}  MAE:{:.4f}  SSIM:{:.4f}  PSNR:{:.4f}  MSE_MAE:{:.4f}  CSI_5:{:.4f}  HSS_5:{:.4f}  CSI_20:{:.4f}  HSS_20:{:.4f}  CSI_40:{:.4f}  HSS_40:{:.4f}\n'. \
                format(epoch + 1, mse_score, mae_score, ssim_score, psnr_score, mse_score + mae_score, scores[5]['csi'],
                       scores[5]['hss'], scores[20]['csi'], scores[20]['hss'], scores[40]['csi'], scores[40]['hss'])
            print(info)
            ...
            print(
                "minMSE: {:.4f}  epoch:{}\nminMAE: {:.4f}  epoch:{}\nminMSE_MAE: {:.4f}  epoch:{}\nmaxSSIM: {:.4f}  epoch:{}\nmaxPSNR: {:.4f}  epoch:{}\nmaxCSI_5: {:.4f}  epoch:{}\nmaxHSS_5: {:.4f}  epoch:{}\nmaxCSI_20: {:.4f}  epoch:{}\nmaxHSS_20: {:.4f}  epoch:{}\nmaxCSI_40: {:.4f}  epoch:{}\nmaxHSS_40: {:.4f}  epoch:{}\nmaxAvgCSI: {:.4f}  epoch:{}\nmaxAvgHSS: {:.4f}  epoch:{}\n".format(
                    ...
                    self.max_metrics[5]['maxCSI'], self.max_metrics[5]['maxCSI_epoch'], self.max_metrics[5]['maxHSS'],
                    self.max_metrics[5]['maxHSS_epoch'],
                    self.max_metrics[20]['maxCSI'], self.max_metrics[20]['maxCSI_epoch'],
                    self.max_metrics[20]['maxHSS'], self.max_metrics[20]['maxHSS_epoch'],
                    self.max_metrics[40]['maxCSI'], self.max_metrics[40]['maxCSI_epoch'],
                    self.max_metrics[40]['maxHSS'], self.max_metrics[40]['maxHSS_epoch'],
                ...
                if epoch + 1 == self.args.epoch:
                    f.write(
                        "minMSE: {:.4f}  epoch:{}\nminMAE: {:.4f}  epoch:{}\nminMSE_MAE: {:.4f}  epoch:{}\nmaxSSIM: {:.4f}  epoch:{}\nmaxPSNR: {:.4f}  epoch:{}\nmaxCSI_5: {:.4f}  epoch:{}\nmaxHSS_5: {:.4f}  epoch:{}\nmaxCSI_20: {:.4f}  epoch:{}\nmaxHSS_20: {:.4f}  epoch:{}\nmaxCSI_40: {:.4f}  epoch:{}\nmaxHSS_40: {:.4f}  epoch:{}\nmaxAvgCSI: {:.4f}  epoch:{}\nmaxAvgHSS: {:.4f}  epoch:{}\n".format(
                            self.minMSE, self.minMSE_epoch, self.minMAE, self.minMAE_epoch, self.minMSE_MAE,
                            self.minMSE_MAE_epoch, self.maxSSIM, self.maxSSIM_epoch, self.maxPSNR, self.maxPSNR_epoch,
                            self.max_metrics[5]['maxCSI'], self.max_metrics[5]['maxCSI_epoch'],
                            self.max_metrics[5]['maxHSS'], self.max_metrics[5]['maxHSS_epoch'],
                            self.max_metrics[20]['maxCSI'], self.max_metrics[20]['maxCSI_epoch'],
                            self.max_metrics[20]['maxHSS'], self.max_metrics[20]['maxHSS_epoch'],
                            self.max_metrics[40]['maxCSI'], self.max_metrics[40]['maxCSI_epoch'],
                            self.max_metrics[40]['maxHSS'], self.max_metrics[40]['maxHSS_epoch'],
